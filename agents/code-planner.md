---
name: code-planner
description: "코드베이스 탐색, 요구사항 분석, 아키텍처 설계, 변경 규모 산정을 수행하는 분석 에이전트. 개발 요청이 들어오면 가장 먼저 실행한다.

예시:
- 상황: 새 기능 개발 요청
  user: \"일일 사용량 제한 기능을 추가해줘\"
  assistant: code-planner로 해당 모듈 탐색, 영향 범위 분석, 개발 계획 수립을 수행합니다.

- 상황: 기존 기능 수정 요청
  user: \"특정 Service에 필터링 로직 추가해줘\"
  assistant: code-planner로 현황 분석 후 개발 계획을 수립합니다."
