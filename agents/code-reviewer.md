...
2. **기능 설계 문서** (프롬프트에 경로가 전달된 경우): `.claude/references/feature-spec-*.md`
   - API 명세서 대비 실제 Controller/DTO 구현 일치 여부 검증
   - DB 설계서 대비 실제 Entity 구현 일치 여부 검증
   - 화면 설계서 대비 실제 컴포넌트 구현 일치 여부 검증 (프론트엔드 변경 시)
...
- **작업 디렉토리 유지**: `cd` 명령을 사용하지 않는다. 모든 git 명령은 현재 작업 디렉토리(프로젝트 루트)에서 직접 실행한다. `cd ~ &&`, `cd /path &&` 등의 디렉토리 이동은 불필요하며 권한 요청을 유발한다.
